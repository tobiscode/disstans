...
+-------------+------------------------+-----------------------------------------------+
|             | ``plot_sigmas``        | If greater than 0, plot the specified amount  |
|             |                        | of standard deviations as shading when        |
|             |                        | uncertainties are present in the timeseries.  |
+-------------+------------------------+-----------------------------------------------+
|             | ``plot_sigmas_alphas`` | Transparency of the uncertainty shading       |
|             |                        | (0-1).                                        |
+-------------+------------------------+-----------------------------------------------+
...
|             |                        | ``std_outliers`` standard deviations away     |
|             |                        | from the reference.                           |
+-------------+------------------------+-----------------------------------------------+
...
|             |                        | percentile) away from the reference's         |
|             |                        | 25th-75th percentile range.                   |
+-------------+------------------------+-----------------------------------------------+
|             | ``min_clean_obs``      | After cleaning, drop timeseries with less     |
|             |                        | than ``min_clean_obs`` observations.          |
+-------------+------------------------+-----------------------------------------------+
|             | ``std_thresh``         | After cleaning, drop timeseries if its        |
|             |                        | standard deviations is higher than            |
|             |                        | ``std_thresh`` (data units).                  |
+-------------+------------------------+-----------------------------------------------+
| ``prior``   | ``mu``                 | Shear modulus μ [GPa] of the elastic half     |
|             |                        | space.                                        |
+-------------+------------------------+-----------------------------------------------+
|             | ``alpha``              | Medium constant α=(λ+μ)/(λ+2μ) [-], where λ   |
|             |                        | is the first Lamé parameter and μ the second  |
|             |                        | one (shear modulus). It is related to the     |
|             |                        | Poisson's ratio ν by α=1/(2-2ν).              |
+-------------+------------------------+-----------------------------------------------+
|             | ``threshold``          | Minimum amount of calculated displacement     |
|             |                        | [mm] that a station needs to surpass in order |
|             |                        | for a step to be added to the model.          |
+-------------+------------------------+-----------------------------------------------+
...
defaults["prior"] = {"mu": 48, "alpha": 0.667, "threshold": 3}
