...
class Network:
    ...
    default_location_path
        If station locations aren't given directly, check for a file with this path for
        the station's location.
        It needs to be an at-least four-column, space-separated text file with the
        entries ``name latitude[°] longitude[°] altitude[m]``. Existing headers have to
        be able to be identified as not being floats in the coordinate columns.
    auto_add
    ...
    auto_add_filter
        If passed alongside ``default_location_path`` and ``auto_add``, network
        instantiation will only add stations within the latitude, longitude polygon
        defined by ``auto_add_filter`` (shape :math:`(\text{num_points}, \text{2})`).
    ...
    def __init__(
        ...
        self.name = str(name)
        """ Network name. """
        self.default_location_path = (
            str(default_location_path) if default_location_path is not None else None
        )
        ...
        These models can be added easily to all stations with
        :meth:`~add_default_local_models`, and will make the JSON export file cleaner.
        ...
        if self.default_location_path is None:
            ...
            for i in range(len(loclines)):
                ...
                try:  # cut the header off
                    ...
                    raise RuntimeError(
                        "Invalid text format for 'default_location_path'."
                    ).with_traceback(e.__traceback__) from e
            # make {station: lla} dictionary
            self._network_locations = {
                line.split()[0]: [float(lla) for lla in line.split()[1:4]]
            ...
            if auto_add:
                ...
                if auto_add_filter is not None:
                    assert (
                        isinstance(auto_add_filter, np.ndarray)
                        and auto_add_filter.shape[1] == 2
                    ), (
                    ...
                    stat_dict = {
                        stat: loc
                        for stat, loc in self._network_locations.items()
    ...
    def station_locations(self) -> pd.DataFrame:
        ...
        return pd.DataFrame(
            data={station.name: station.location for station in self},
            index=["Latitude [°]", "Longitude [°]", "Altitude [m]"],
        ).T
    ...
    def get_stations_with(
        self, ts_description: str, mdl_description: str | None = None
    ) -> list[str]:
        """
        ...
        assert (mdl_description is None) or isinstance(
            mdl_description, str
        ...
        for name in station_list:
            if (ts_description not in self[name]) or (
                (mdl_description is not None)
                and (mdl_description not in self[name].models[ts_description])
            ):
                station_list.remove(name)
        return station_list

    def export_network_ts(
        self,
        ts_description: str | tuple[str, str],
        subset_stations: list[str] | None = None,
    ) -> dict[str, Timeseries]:
        """
        Collects a specific timeseries from all stations and returns them in a
        dictionary of network-wide :class:`~disstans.timeseries.Timeseries` objects.
        ...
        if isinstance(ts_description, str):
            ...
            raise ValueError(
                "Unrecognized input type for 'ts_description': "
        ...
        for name in stat_names:
            ...
            if ts_is_timeseries and (ts_description in station.timeseries):
                ...
                (not ts_is_timeseries)
                and (ts_model_ts in station.fits)
        ...
        if df_dict == {}:
            raise ValueError(
                f"No data found in network '{self.name}' "
    ...
    def import_network_ts(
        self, ts_description: str, dict_of_timeseries: dict[str, Timeseries]
    ) -> None:
        """
        Distributes a dictionary of network-wide
        :class:`~disstans.timeseries.Timeseries` objects onto the network stations.

        Parameters
        ----------
        ts_description
            :class:`~disstans.timeseries.Timeseries` description where the data will be
            placed.
        dict_of_timeseries
            Dictionary with the data components of the ``ts_description`` timeseries
            as keys and :class:`~disstans.timeseries.Timeseries` objects as values
        ...
        network_df = pd.concat(
            {name: ts.data for name, ts in dict_of_timeseries.items()}, axis=1
        )
        network_df.columns = network_df.columns.swaplevel(0, 1)
        network_df.sort_index(level=0, axis=1, inplace=True)
        ...
        for name in network_df.columns.levels[0]:
            self.stations[name].add_timeseries(
                ts_description,
                Timeseries(network_df[name].dropna(), src, data_unit, data_cols),
            )
    ...
    def add_station(self, name: str, station: Station) -> None:
        ...
        and ``station`` a :class:`~disstans.station.Station` instance, then the
        ...
        if not isinstance(station, Station):
            raise TypeError(
                "Cannot add new station: 'station' is not a Station object."
            )
        if name in self.stations:
            warn(
                f"Overwriting station '{name}'.", category=RuntimeWarning, stacklevel=2
            )
        self.stations[name] = station

    def create_station(
        self, name: str, location: list[float] | np.ndarray | None = None
    ) -> None:
        """
        ...
        if location is not None:
            assert isinstance(location, list) and all(
                [isinstance(loc, float) for loc in location]
            ), (
                "'location' needs to be a list of latitude, longitude and altitude "
                f"floats, got {location}."
            )
        elif name in self._network_locations:
            location = self._network_locations[name]
        else:
            raise ValueError(
                f"'location' was not passed to create_station, and {name} could "
    ...
    def remove_station(self, name: str) -> None:
        ...
        Example
        -------
        If ``net`` is a :class:`~Network` instance and ``name`` is the name of an
        existing station, then the following two are equivalent::
        ...
        if name not in self.stations:
            warn(
                f"Cannot find station '{name}', couldn't delete.",
    ...
    def from_json(
        ...
        """
        Create a :class:`~disstans.network.Network` instance from a JSON configuration
        file.
        ...
        # create stations
        for station_name, station_cfg in tqdm(
            net_arch["stations"].items(),
            ascii=True,
            ...
            if "location" in station_cfg:
                ...
                warn(
                    f"Skipped station '{station_name}'  because location information "
                    "is missing.",
                    stacklevel=2,
                )
                continue
            station = Station(
                name=station_name, location=station_loc, **station_kw_args
            )
            ...
            for ts_description, ts_model_dict in station_cfg["models"].items():
                if ts_description in station.timeseries:
                    station.add_local_model_kwargs(
                        ts_description=ts_description, model_kw_args=ts_model_dict
                    )
                else:
        ...
        for stat_name, station in self.stations.items():
            ...
            for model_description, mdl in self.default_local_models.items():
                for ts_description in stat_arch["models"]:
                    if (
                        model_description in stat_arch["models"][ts_description].keys()
                    ) and (
                        mdl == stat_arch["models"][ts_description][model_description]
                    ):
    ...
    def update_default_local_models(self, models: dict[str, dict[str, Any]]) -> None:
        ...
        models
            ...
            (see :func:`~disstans.models.check_model_dict`).
        """
        disstans_models.check_model_dict(models)
        self.default_local_models.update(models)

    def add_default_local_models(
        self, ts_description: str, models: list[str] | None = None
    ) -> None:
        """
        ...
        if models is None:
            ...
            if not isinstance(models, str) and not (
                ...
                raise ValueError(
                    "'models' must be None, a string or a list of strings, "
                    f"got {models}."
                )
            ...
            local_models_subset = {
                name: model
                for name, model in self.default_local_models.items()
                if name in models
            }
        for station in self:
            for model_description, model_cfg in local_models_subset.items():
                local_copy = deepcopy(model_cfg)
                mdl = getattr(disstans_models, local_copy["type"])(
                    **local_copy["kw_args"]
                )
    ...
    def add_unused_local_models(
        ...
        import of a JSON file, but there is no associated information for that
        timeseries itself. Usually, this happens when after the network is loaded, the
        target timeseries still needs to be added.
        ...
        assert (
            ...
            or (isinstance(models, list) and all([isinstance(m, str) for m in models]))
        ...
        for station in self:
            if hidden_ts in station.unused_models:
                if models is None:
                    ...
                    local_models_subset = {
                        name: model
                        for name, model in station.unused_models[hidden_ts].items()
                        if name in models
                    }
                station.add_local_model_kwargs(
                    ts_description=target_ts, model_kw_args=local_models_subset
                )
    ...
    def copy_timeseries(
        ...
        for station in self:
            if origin_ts in station.timeseries.keys():
                ...
                if with_models:
                    station.models[target_ts] = station.models[origin_ts].copy(
                        **mdl_copy_kw_args
                    )
    ...
    def load_maintenance_dict(
        ...
        ----------
        maint_dict
            Dictionary of structure ``{station_name: [steptimes]}`` where
            ``station_name`` is the station name as present in the Network object, and
            ``steptimes`` is a list of either datetime-like strings or
        ...
        """
        assert all(
            [isinstance(station, str) for station in maint_dict.keys()]
        ), f"Keys in 'maint_dict' must be strings, got {maint_dict.keys()}."
        assert all(
            [isinstance(steptimes, list) for steptimes in maint_dict.values()]
            ...
            if station in self.stations:
                if only_active:
                    ...
                    localsteps = [
                        st
                        for st in steptimes
                        if ((pd.Timestamp(st) >= tmin) and (pd.Timestamp(st) <= tmax))
                    ]
                else:
                    localsteps = steptimes
                if len(localsteps) > 0:
                    self[station].add_local_model(
                        ts_description,
                        model_description,
    ...
    def freeze(
        ...
        for station in self:
            if ts_description in station:
                station.models[ts_description].freeze(
                    model_list=model_list, zero_threshold=zero_threshold
                )

    def unfreeze(
        self, ts_description: str, model_list: list[str] | None = None
    ) -> None:
        """
    ...
    def fit(
        ...
        **kw_args,
        ...
        solver
            ...
            name in :mod:`~disstans.solvers`, otherwise will use the passed function as
            a solver (which needs to adhere to the same input/output structure as the
        ...
        **kw_args
        ...
        Example
        -------
        If ``net`` is a :class:`~Network` instance, ``'mydata'`` is the timeseries to
        fit, and ``mysolver`` is the solver to use, then the following two are
        equivalent::
            ...
            for station in net:
                ...
                station_models.read_parameters(
                    sol.parameters_zeroed, sol.covariances_zeroed
                )
        ...
        assert any(
            [ts_description in station.timeseries.keys() for station in self]
        ...
        progress_desc = (
            str(progress_desc) if progress_desc else "Fitting station models"
        )
        station_names = [
            name
        ...
        iterable_inputs = (
            (
                ...
                kw_args if local_input == {} else {**kw_args, **local_input[stat_name]},
            )
        ...
        for i, sol in enumerate(
            tqdm(
                parallelize(self._fit_single_station, iterable_inputs),
                desc=progress_desc,
            ...
            if not sol.converged:
                warn(
                    f"Fitting did not converge for timeseries {ts_description} "
                    f"at {station_names[i]}",
                    category=RuntimeWarning,
                    stacklevel=2,
                )
            self[station_names[i]].models[ts_description].read_parameters(
                sol.parameters_zeroed, sol.covariances_zeroed
            )
    ...
    @staticmethod
    def _fit_single_station(
        parameter_tuple: tuple[Callable, Timeseries, ModelCollection, dict],
    ) -> Solution:
    ...
    def spatialfit(
        ...
        ``spatial_reweight_max_rms`` *or* ``spatial_reweight_max_changed`` are
        satisfied, for all models in ``spatial_l0_models``.
        ...
        penalty
            ...
            every iteration. For the reweighted models, and if
            ``local_reweight_coupled=True`` (default), this is just the penalty at the
            first iteration. After that, the penalties are largely controlled by
            ``reweight_func``. If ``local_reweight_coupled=False``, the penalty is
            applied on top of the updated weights at each iteration.
        ...
        spatial_reweight_max_rms
            Stop the spatial iterations early if the difference in the RMS (Root Mean
            Square) of the change of the parameters between reweighting iterations is
            less than ``spatial_reweight_max_rms``.
        spatial_reweight_max_changed
            ...
            ``spatial_reweight_max_changed`` is given as the percentage of changed over
            total parameters (including all models and components).
        ...
        spatial_reweight_agg_comps
            ...
            component, i.e., the distance-weighted median of the weights will be
            calculated for each component. If ``True``, a single median is calculated
            from the aggregated weights in all data components, and then used for all
            components.
        dist_num_avg
        ...
        local_l0_models
            Names of models that should carry over their weights from one solver
            iteration to the next, but should not be reweighted spatially, resulting in
        ...
        keep_mdl_res_as
            If ``extended_stats=True``, the network's models are evaluated, and a model
            fit and residuals timeseries are created. Between iterations, they are
            removed by default, but passing this parameter a 2-element tuple of strings
            keeps the evaluated model and residual timeseries after the last iteration.
            This effectively allows the user to skip
            :meth:`~disstans.network.Network.evaluate` (for ``output_description``) and
            the calculation of the residual as done with ``residual_description`` by
            :meth:`~disstans.network.Network.fitevalres` after finishing the spatial
            fitting.
        return_solutions
            If ``True``, return a dictionary of all solutions produced by the calls to
            the solver function.
        zero_threshold
            When calculating statistics, assume parameters with absolute values smaller
            than ``zero_threshold`` are effectively zero.
        cov_zero_threshold
            Covariance zero threshold, see :func:`~disstans.solvers.lasso_regression`.
        num_threads_evaluate
            If ``extended_stats=True`` and ``formal_covariance=True``, there will be
            calls to :meth:`~evaluate` that will estimate the predicted variance,
            which will be memory-intensive if the timeseries are long. Using the same
            number of threads as defined in :attr:`~disstans.config.defaults` might
            therefore exceed the available memory on the system. This option allows to
            set a different number of threads or disable parallelized processing
            entirely for those calls.
            Defaults to ``None``, which uses the same setting as in the defaults.
        roll_mean_kernel
            Only used if ``extended_stats=True``. This is the kernel size that gets used
            in the analysis of the residuals between each fitting step.
        cvxpy_kw_args
        ...
        - ``'arr_uniques'`` (:class:`~numpy.ndarray`):
          Array of shape
          :math:`(\text{spatial_reweight_iters}+1, \text{num_components})`
        ...
        - ``'dict_num_changed'`` (:class:`~dict`):
          Dictionary that for each reweighted model and contains a list (of length
          ``spatial_reweight_iters``) of the number of reweighted parameters that
          changed from zero to non-zero or vice-versa.
        - ``'list_res_stats'`` (:class:`~list`):
          (Only present if ``extended_stats=True``.) List of the results dataframe
          returned by :meth:`~analyze_residuals` for each iteration.
        ...
        .. [riel14] Riel, B., Simons, M., Agram, P., & Zhan, Z. (2014).
           *Detecting transient signals in geodetic time series
           using sparse estimation techniques*.
        ...
        assert num_stations > 1, (
            "The number of stations in the network that "
            "contain the timeseries must be more than one."
        )
        assert isinstance(spatial_l0_models, list) and all(
            [isinstance(mdl, str) for mdl in spatial_l0_models]
        ), (
            "'spatial_l0_models' must be a list of model name strings, got "
            + f"{spatial_l0_models}."
        )
        assert isinstance(spatial_reweight_iters, int) and (
            spatial_reweight_iters > 0
        ), (
            "'spatial_reweight_iters' must be an integer greater than 0, got "
            + f"{spatial_reweight_iters}."
        )
        assert float(spatial_reweight_max_rms) >= 0, (
            "'spatial_reweight_max_rms' needs "
            f"to be greater or equal to 0, got {spatial_reweight_max_rms}."
        )
        assert 0 <= float(spatial_reweight_max_changed) <= 1, (
            "'spatial_reweight_max_changed' "
            f"needs to be between 0 and 1, got {spatial_reweight_max_changed}."
        )
        if local_l0_models != []:
            assert isinstance(local_l0_models, list) and all(
                [isinstance(mdl, str) for mdl in local_l0_models]
            ), (
                "'local_l0_models' must be a list of model name strings, got "
                + f"{local_l0_models}."
            )
        all_reweight_models = set(spatial_l0_models + local_l0_models)
        assert len(all_reweight_models) == len(spatial_l0_models) + len(
            local_l0_models
        ), (
            "'spatial_l0_models' "
        ...
        assert isinstance(reweight_func, ReweightingFunction), (
            "'reweight_func' needs to be a ReweightingFunction, "
            f"got {type(reweight_func)}."
        )
        ...
        station_lonlat = np.stack(
            [np.array(self[name].location)[[1, 0]] for name in station_names]
        )
        all_distances = np.empty((num_stations, num_stations))
        net_avg_closests = []
        for i in range(num_stations):
            all_distances[i, :] = np.array(
                geoid.inverse(station_lonlat[i, :].reshape(1, 2), station_lonlat)
            )[:, 0]
            net_avg_closests.append(
                np.sort(all_distances[i, :])[
                    1 : min(num_stations, 1 + dist_num_avg)
                ].mean()
            )
        distance_weights = np.exp(
            -all_distances
            / np.clip(
                np.array(net_avg_closests), a_min=dist_weight_min, a_max=dist_weight_max
            ).reshape(1, -1)
        )
        ...
        if verbose:
            ...
            tqdm.write(
                "Distance percentiles in km (5-50-95): "
        ...
        num_total = sum(
            [
                s.models[ts_description][m].parameters.size
                for s in valid_stations.values()
                ...
                [
                    np.sum(
                        np.any(
                            np.stack(
                                [
                                    np.abs(s.models[ts_description][m].parameters)
                                    > zero_threshold
                                    for s in valid_stations.values()
                                ]
                            ),
        ...
        num_nonzero = sum(
            [
                (
                    np.abs(s.models[ts_description][m].parameters.ravel())
                    > zero_threshold
                ).sum()
                for s in valid_stations.values()
                for m in all_reweight_models
        ...
        if verbose:
            tqdm.write(
                f"Number of reweighted non-zero parameters: {num_nonzero}/{num_total}"
            )
            tqdm.write(
                "Number of unique reweighted non-zero parameters per component: "
                + str(num_uniques.tolist())
            )

        # initialize the other statistics objects
        assert num_components == num_uniques.size, (
            f"Expected {num_components} data "
        ...
        dict_rms_diff = {
            m: [np.nan for _ in range(spatial_reweight_iters)]
            for m in all_reweight_models
        }
        dict_num_changed = {
            m: [np.nan for _ in range(spatial_reweight_iters)]
        ...
        old_params = {
            mdl_description: Solution.aggregate_models(
                results_dict=solutions,
        ...
        if extended_stats:
            ...
            dict_cors_means = {
                mdl_description: [] for mdl_description in all_reweight_models
            }
            dict_cors_det = {
                mdl_description: [] for mdl_description in all_reweight_models
            }
            ...
            def save_extended_stats(is_last=False):
                if (
                    is_last
                    and isinstance(keep_mdl_res_as, tuple)
                    and (len(keep_mdl_res_as) == 2)
                ):
                    keep_last = True
                ...
                self.evaluate(
                    ts_description, output_description=iter_name_fit, no_pbar=no_pbar
                )
                if num_threads_evaluate is not None:
                ...
                for mdl_description in all_reweight_models:
                    net_mdl_df = list(
                        self.export_network_ts(
                            (ts_description, mdl_description)
                        ).values()
                    )
                    cormats = [mdl_df.df.corr().abs().values for mdl_df in net_mdl_df]
                    cormats_means = [
                        np.nanmean(np.ma.masked_equal(np.triu(cormat, 1), 0))
                    ...
                    for i in range(len(net_mdl_df)):
                        ...
                        for j in range(raw_values.shape[1]):
                            raw_values[index_valid[:, j], j] = sp.signal.detrend(
        ...
        for i in range(spatial_reweight_iters):
            ...
            new_net_weights = {
                statname: {"reweight_init": {}} for statname in station_names
            }
            # reweighting spatial models
            for mdl_description in spatial_l0_models:
                (stacked_weights,) = Solution.aggregate_models(
                    results_dict=solutions,
                    mdl_description=mdl_description,
                    key_list=station_names,
                    stack_weights=True,
                ...
                if np.any(stacked_weights):
                    if verbose:
                        ...
                        # print percentiles
                        percs = [
                            np.nanpercentile(stacked_weights, q) for q in [5, 50, 95]
                        ]
                        tqdm.write(
                            "Weight percentiles (5-50-95): "
                            f"[{percs[0]:.11g}, {percs[1]:.11g}, {percs[2]:.11g}]"
                        )
                    # if aggregating components, reshape and concatenate axes
                    if spatial_reweight_agg_comps:
                        stacked_weights = (stacked_weights.transpose(0, 2, 1)).reshape(
                            num_stations * num_components, -1
                        )
                    # now apply the spatial median to parameter weights
                    for station_index, name in enumerate(station_names):
                        new_weights = weighted_median(
                            stacked_weights,
                            distance_weights[station_index, :],
                            percentile=spatial_reweight_percentile,
                        )
                        if spatial_reweight_agg_comps:
                            new_weights = np.repeat(
                                new_weights.reshape(-1, 1), num_components, axis=1
                            )
                        new_net_weights[name]["reweight_init"][
                            mdl_description
                        ] = new_weights
                else:  # stacking failed, keep old weights
                    warn(
                        f"{mdl_description} cannot be stacked, reusing old weights.",
                        stacklevel=2,
                    )
            ...
            # for models that were regularized but not spatially or continuously
            # reweighted, set the initial weights to penalty
            # (such that it keeps a constant penalty)
            for name in station_names:
                for mdl_description, mdl in (
                    self[name].models[ts_description].collection.items()
                ):
            ...
            solutions = self.fit(
                ...
                progress_desc=None if verbose else f"Fit after {i + 1} reweightings",
                no_pbar=no_pbar,
                penalty=penalty,
                reweight_max_iters=local_reweight_iters,
                reweight_func=reweight_func,
            ...
            if verbose:
                tqdm.write(
                    "Number of reweighted non-zero parameters: "
            ...
            early_stop = True
            for mdl_description in all_reweight_models:
                (stacked_params,) = Solution.aggregate_models(
                    results_dict=solutions,
                    mdl_description=mdl_description,
                    key_list=station_names,
                    stack_parameters=True,
                ...
                num_changed = np.logical_xor(
                    np.abs(old_params[mdl_description]) < zero_threshold,
                    np.abs(stacked_params) < zero_threshold,
                ).sum()
                ...
                if verbose:
                    tqdm.write(
                        f"RMS difference of '{mdl_description}' parameters = "
                        f"{rms_diff:.11g} ({num_changed} changed)"
                    )
            # save extended statistics
            if extended_stats:
                save_extended_stats(
                    is_last=early_stop or (i == spatial_reweight_iters - 1)
                )
    ...
    def evaluate(
        ...
        to evaluate the models for, then the following two are equivalent::
            ...
            # long version, not parallelized,
            # and not creating a new independent timeseries
            for station in net:
                station_ts = station.timeseries['mydata']
        ...
        progress_desc = (
            str(progress_desc) if progress_desc else "Evaluating station models"
        )
        station_names = [
            name
            for name, station in self.stations.items()
        ...
        iterable_inputs = (
            (
                (
                    self[stat_name].timeseries[ts_description].time
        ...
        for i, (sumfit, mdlfit) in enumerate(
            tqdm(
                parallelize(self._evaluate_single_station, iterable_inputs),
                desc=progress_desc,
                total=len(station_names),
                ascii=True,
                unit="station",
            ...
            # add overall model
            ts = stat.add_fit(
                ts_description, sumfit, return_ts=(output_description is not None)
            )
    ...
    @staticmethod
    def _evaluate_single_station(
        parameter_tuple: tuple[pd.Series | pd.DatetimeIndex, ModelCollection],
    ...
    def fitevalres(
        ...
        **kw_args
        ...
        -------
        If ``net`` is a :class:`~Network` instance, ``'mydata'`` is the timeseries to
        fit and evaluate the models for, ``mysolver`` is the solver to use, ``'myfit'``
        should contain the model fit, and the residuals should be put into the
        timeseries ``'myres'``, then the following two are equivalent::
        ...
        if progress_desc is not None:
            assert isinstance(progress_desc, tuple) and (len(progress_desc) == 2), (
                "'progress_desc' needs to be None or a 2-element tuple, "
                f"got {progress_desc}."
            )
        else:
            progress_desc = (None, None)
        # fit
        possible_output = self.fit(
            ts_description=ts_description,
            solver=solver,
    ...
    def call_func_ts_return(
        ...
        **kw_args,
        ...
        func
            ...
            :mod:`~disstans.processing`, otherwise the function will be assumed to
            adhere to the same input and output format than the included ones.
        ts_in
        ...
        **kw_args
            ...
            if isinstance(func, str):
                try:
                    ...
                    raise AttributeError(
                        f"'{func}' can not be found "
                        "as a function in disstans.processing."
                    ).with_traceback(e.__traceback__) from e
            else:
                raise RuntimeError(
                    f"'{func}' needs to be a function or a string "
        ...
        for i, result in enumerate(
            tqdm(
                parallelize(self._single_call_func_ts_return, iterable_inputs),
                desc="Processing station timeseries with " f"'{func.__name__}'",
                total=len(station_names),
                ascii=True,
                unit="station",
                disable=no_pbar,
    ...
    @staticmethod
    def _single_call_func_ts_return(
        parameter_tuple: tuple[Callable, Station, str, dict],
    ) -> Timeseries:
        func, station, ts_description, kw_args = parameter_tuple
        ts_return = func(station.timeseries[ts_description], **kw_args)
        return ts_return

    def call_netwide_func(
        self, func: str | Callable, ts_in: str, ts_out: str | None = None, **kw_args
    ) -> None:
        """
        A convenience wrapper for functions that operate on an entire network's
        timeseries at once.
        ...
        **kw_args
        ...
        Example
        -------
        If ``net`` is a :class:`~Network` instance, ``'input'`` and ``'output'`` are the
        names of the input and output timeseries, respectively, and ``func`` is the
        function to use, then the following two are equivalent::
    ...
    def call_func_no_return(
        self, func: str | Callable, no_pbar: bool = False, **kw_args
    ) -> None:
        """
        A convenience wrapper that for each station in the network, calls a given
        function on each station. Also provides a progress bar.
        ...
        func
            Function to use. If provided with a string, will check for that function in
            :mod:`~disstans.processing`, otherwise the function will be assumed to
            adhere to the same input format than the included ones.
        no_pbar
            Suppress the progress bar with ``True``.
        **kw_args
            Additional keyword arguments to be passed onto ``func``.
    ...
    def math(
        self, result: str, left: str, operator: Literal["+", "-", "*", "/"], right: str
    ) -> None:
        """
        ...
        operator
            Operator symbol (see :meth:`~disstans.timeseries.Timeseries.prepare_math`
        ...
        Example
        -------
        If ``net`` is a :class:`~Network` instance, and we want to calculate the
        residual ``'res'`` between the data in timeseries ``'data'`` and the model in
        the timeseries ``'model'``, then the following two are equivalent::
        ...
        for station in self:
            if (left in station.timeseries) and (right in station.timeseries):
                station[result] = eval(
                    f"station['{left}'] {operator} station['{right}']"
                )

    def analyze_residuals(self, ts_description: str, **kw_args) -> pd.DataFrame:
        """
        ...
        **kw_args
        ...
        metrics_lists = [
            station.analyze_residuals(ts_description, **kw_args)
        ...
        metrics_arr = np.array(
            [
                [component for metric in sta_metrics.values() for component in metric]
        ...
        metrics = list(
            dict.fromkeys(
                [metric for sta_metrics in metrics_lists for metric in sta_metrics]
            )
        )
        components = list(
            dict.fromkeys(
        ...
        metrics_components = pd.MultiIndex.from_product(
            [metrics, components], names=["Metrics", "Components"]
        )
        # create and return a multi-level DataFrame
        return pd.DataFrame(
            metrics_arr, index=list(valid_stations.keys()), columns=metrics_components
        ).rename_axis("Station")

    def decompose(
        self, ts_in: str, ts_out: str | None = None, **decompose_kw_args
        ...
        **decompose_kw_args
            ...
            :func:`~disstans.processing.decompose`.
        ...
        model
            ...
            The Timeseries objects have the station names as columns, and their
            observation record length is the joint length of all stations in the
            network, :math:`\text{num_observations}`. If a station is missing a certain
            timestamp, that value is ``NaN``.
        spatial
    ...
    def _subset_stations(
        ...
        if subset_stations is None:
            ...
            assert isinstance(subset_stations, list) and all(
                [isinstance(name, str) for name in subset_stations]
            ), (
                "'subset_stations' needs to be a list of station names, "
                f"got {subset_stations}."
            )
            stat_names_in = sorted(
                list(set(stations_valid).intersection(set(subset_stations)))
            )
        # parse the extrapolate option
        if isinstance(extrapolate, list):
            assert all(
                [isinstance(name, str) for name in extrapolate]
            ), f"The 'extrapolate' list contains non-string entries: {extrapolate}."
            stat_names_out = sorted(
                list(set(stations_valid).intersection(set(extrapolate)))
            )
    ...
    def _get_2comp_pars_covs(
        ...
        pars = np.stack(
            [
                station.models[ts_description][mdl_description].par[index, comps]
        ...
        if use_vars:
            covs = np.stack(
                [
                    station.models[ts_description][mdl_description].var[index, comps]
                    for station in stat_list_in
                ],
                axis=0,
            )
            if use_covs:
                covlist = [
                    station.models[ts_description][mdl_description].get_cov_by_index(
                        index
                    )[comps]
                    for station in stat_list_in
                ]
    ...
    def hom_velocity_field(
        ...
        # input check
        assert (
            isinstance(comps, tuple)
            and len(comps) == 2
            and all([isinstance(c, int) for c in comps])
        ), f"'comps' needs to be a length-2 tuple of integers, got {comps}."
        assert (
            isinstance(use_parts, tuple)
            and (len(use_parts) == 3)
            and all([isinstance(p, bool) for p in use_parts])
            and any(use_parts)
        ), (
        ...
        stat_names_valid, stat_names_in, stat_names_out = self._subset_stations(
            stat_names_valid, subset_stations, extrapolate
        )
        ...
        if reference_station is not None:
            if reference_station in stat_names_in:
                ...
                reference_index = 0
                warn(
                    f"Reference station '{reference_station}' is not included in the"
                    " input station list, using "
                    f"'{stat_names_in[reference_index]}' instead."
                )
        else:
            reference_index = 0
        ...
        vels, weights = self._get_2comp_pars_covs(
            stat_list_in,
            ts_description,
            mdl_description,
            linear_index,
        ...
        v_O, epsilon, omega = get_hom_vel_strain_rot(
            lon_lat_in, vels, weights, utmzone=utmzone, reference=[lon0, lat0]
        )
    ...
    def euler_rot_field(
        ...
        -------
        df_v_pred
            DataFrame containing the Euler pole modeled velocities at each station
            (rows) in the East and North components (columns).
        rotation_vector
            Rotation vector [rad/time] containing the diagonals of the
            :math:`3 \times 3` rotation matrix specifying the Euler pole in cartesian,
            ECEF coordinates.
        rotation_covariance
        ...
        rotation_vector, rotation_covariance = estimate_euler_pole(
            lon_lat_in, vels, weights
        )
        # get ECEF coordinates for all output stations
        crs_lla = ccrs.Geodetic()
        crs_xyz = ccrs.Geocentric()
        stat_xyz = crs_xyz.transform_points(
            crs_lla, lon[ix_out], lat[ix_out], alt[ix_out]
        )
        # predict rotation on surface of sphere
        v_pred_xyz = np.cross(rotation_vector, stat_xyz)
        v_pred_enu = np.stack(
            [
                (R_ecef2enu(lo, la) @ v_pred_xyz[i, :])
                for i, (lo, la) in enumerate(zip(lon[ix_out], lat[ix_out]))
            ]
        )
        # return as DataFrame
        df_v_pred = pd.DataFrame(
            data=v_pred_enu[:, :2], index=stat_names_out, columns=v_pred_cols
        )
    ...
    def get_trend_change(
        ...
        **kw_args,
        ...
        """
        For each station, calculates the (linear) trends of a timeseries before and
        after given timestamps, and return for each one whether the trend increased,
        ...
        spatial_mean
            If a value larger than ``0``, use this distance in km as the exponential
            smoothing factor for a spatially-weighted mean before returning the trend
            changes.
        **kw_args
        ...
        try:
            ...
            raise KeyError(
                "Missing required keyword argument 'ts_description'."
            ).with_traceback(e.__traceback__) from e
        # warn if no station available with that timeseries
        if len(check_stations) == 0:
            raise RuntimeError(
                "No stations found that contain timeseries "
        ...
        tc_dict = {
            sta_name: sta.get_trend_change(**kw_args)
            for sta_name, sta in tqdm(
                check_stations.items(),
                desc="Calculating trends",
                unit="Station",
                ascii=True,
            )
        }
        ...
        if spatial_mean:
            ...
            tc_arr = np.ma.masked_invalid(
                np.stack(list(tc_dict.values())).astype(float)
            )
            # get distances between stations
            stat_lonlats = np.stack(
                [self.stations[sta_name].location[1::-1] for sta_name in tc_dict.keys()]
            )
            geoid = cgeod.Geodesic()
            ...
            for i in range(num_stations):
                all_distances[i, :] = np.array(
                    geoid.inverse(stat_lonlats[i, :][None, :], stat_lonlats)
                )[:, 0]
            # convert to weights
            ...
            for i in range(num_stations):
                tc_arr_means[i, :, :] = np.ma.average(
                    tc_arr, axis=0, weights=distance_weights[i, :]
                )
            ...
            if return_signs:
                tc_arr_means_obj[~np.isnan(tc_arr_means)] = tc_arr_means_obj[
                    ~np.isnan(tc_arr_means)
                ].astype(int)
            # pack array into dictionary
            tc_dict = {
                sta_name: tc_arr_means_obj[i, :, :].tolist()
    ...
    def _create_map_figure(
        ...
        stat_points = ax_map.scatter(
            stat_lons,
            stat_lats,
            s=20,
            facecolor="C0",
            edgecolor=default_station_edges,
            marker="o",
            transform=proj_lla,
            zorder=1000,
        )
        ...
        if annotate_stations:
            ...
            if isinstance(annotate_stations, float) or isinstance(
                annotate_stations, str
            ):
        ...
        if gui_settings["coastlines_show"]:
            ax_map.coastlines(
                color="white" if map_underlay else "black",
    ...
    def graphical_cme(
        ...
        **decompose_kw_args,
        ...
        annotate_stations
            ...
            If a float or a string, add the station names to the map with the font size
            set as required by :class:`~matplotlib.text.Text`.
        save
            If ``True``, save the map and timeseries plots to the current folder.
        save_kw_args
            Additional keyword arguments passed to
            :meth:`~matplotlib.figure.Figure.savefig`, used when ``save=True``.
        gui_kw_args
        ...
        **decompose_kw_args
        ...
        model, spatial, temporal = self.decompose(
            ts_in, ts_out=ts_out, **decompose_kw_args
        )
        ...
        fig_map, ax_map, _, proj_lla, _, _, _, _ = self._create_map_figure(
            gui_settings, annotate_stations, center_longitude=self.mean_longitude
        )
        if ndim == 1:
            quiv = ax_map.quiver(
                latlonenu[:, 1],
        ...
        key_length = np.median(
            np.sqrt(np.sum(np.atleast_2d(latlonenu[:, 2 : 2 + ndim_max2] ** 2), axis=1))
        )
        ax_map.quiverkey(
            quiv,
            0.9,
    ...
    def gui(
        ...
        - a window with a map of all stations, underlain optionally with coastlines
          and/or imagery, and
        - another window which will show a station's timeseries including all fitted
          models.
        ...
        - restrict the output to a timewindow showing potential steps from multiple
          sources,
        ...
        save
            ...
            is also created, save this as well. The output directory is the current
            folder. Ignored if ``stepdetector`` is set. Suppresses all interactive
            figures. If ``save`` is a string, it will be included in the output file
            name for easier referencing.
        save_map
        ...
        scalogram_kw_args
            ...
            The dictionary has to contain ``'ts'`` and ``'model'`` keys. The string
            values are the names of the timeseries and associated model that are of the
        ...
        stepdetector
            Passing this dictionary will enable the plotting of events related to
            possible steps, both on the map (in case of an earthquake catalog) and in
            the timeseries (in case of detected steps by
            :class:`~disstans.processing.StepDetector` or a maintenance catalog).
            To reduce cluttering, it will also only show a subset of each timeseries,
            centered around consecutive probable steps. Using the terminal, one can
            cycle through each period.
            The ``stepdetector`` dictionary must contain the keys ``'plot_padding'``
            (which determines how many days before and after to include in the
            plotting), ``'step_table'`` and ``'step_ranges'`` (both returned by
            :meth:`~disstans.processing.StepDetector.search_network`),
            ``'step_padding'`` (which determines how many days before and after the
            ``'step_ranges'`` should be scanned for possible steps), optionally
            ``'catalog'`` and ``'eqcircle'`` (an earthquake catalog of same style as
            used in :mod:`~disstans.earthquakes` and a maximum distance of stations to
            earthquakes of magnitude less than 7.5), and optionally, ``'maint_table'``
            (a maintenance table as parsed by
            :func:`~disstans.tools.parse_maintenance_table`).
        trend_kw_args
            ...
            passed to ``gui()`` will be used instead. If the number of components
            available is 3 or more, only the first two will be used. If two components
            are plotted, they correspond to the East and North components. If only one
            component is plotted (for example for vertical motion), it will be plotted
            as the North component.
        analyze_kw_args
            If provided and non-empty, call
            :meth:`~disstans.station.Station.analyze_residuals`
        ...
        rms_on_map
            ...
            a residual timeseries' root-mean-squares to color the station markers on the
            map. The dictionary must include the key ``'ts'`` (the residual timeseries'
            name), and can optionally include the keys ``'comps'`` (a list of components
            to combine for the RMS, defaults to all components), ``'c_max'`` (maximum
            colormap range, defaults to maximum of the RMS), ``'t_start', 't_end'``
            (to restrict the time window, defaults to ``analyze_kw_args`` if given,
            otherwise the entire timeseries), and ``'orientation'`` (for the colorbar
            orientation).
        gui_kw_args
            Override default GUI settings of :attr:`~disstans.config.defaults`.
        """
        ...
        if isinstance(timeseries, str):
            ...
            assert (timeseries is None) or (
                isinstance(timeseries, list)
                and all([isinstance(t, str) for t in timeseries])
            ), (
                "'timeseries' must be None, a string, or list of strings, "
                f"got '{timeseries}'."
            )
        ...
        if scalogram_kw_args is not None:
            assert (
                isinstance(scalogram_kw_args, dict)
                and all([key in scalogram_kw_args for key in ["ts", "model"]])
                and all(
                    [isinstance(scalogram_kw_args[key], str) for key in ["ts", "model"]]
                )
            ), (
        ...
        assert not (
            (mark_events is not None) and stepdetector
        ...
        if rms_on_map:
            # check input
            assert (
                isinstance(rms_on_map, dict)
                and ("ts" in rms_on_map)
                and isinstance(rms_on_map["ts"], str)
            ), (
            ...
            for i, stat in enumerate(self):
                rms[i] = (
                    np.linalg.norm(
                        stat.analyze_residuals(rms_on_map["ts"], **rms_kw_args)["RMS"]
                    )
                    if rms_on_map["ts"] in stat.timeseries
                    else 0
                )
            # make colormap
            cmax = rms_on_map.get("c_max", np.max(rms))
            rms_cmap = mpl.cm.ScalarMappable(
                cmap=scm.lajolla_r, norm=mpl.colors.Normalize(vmin=0, vmax=cmax)
            )
        ...
        if trend_kw_args:
            assert (
                "ts_description" in trend_kw_args
        ...
        # set map extent
        ax_map_xmin, ax_map_xmax, ax_map_ymin, ax_map_ymax = ax_map.get_extent()
        cur_lon_min, cur_lat_min = proj_lla.transform_point(
            ax_map_xmin, ax_map_ymin, proj_gui
        )
        cur_lon_max, cur_lat_max = proj_lla.transform_point(
            ax_map_xmax, ax_map_ymax, proj_gui
        )
        ...
        if mark_events is not None:
            if isinstance(mark_events, pd.DataFrame):
                assert all(
                    [col in mark_events.columns for col in ["station", "time"]]
                ), "'mark_events' needs to contain the columns 'station' and 'time'."
            elif isinstance(mark_events, list) and all(
                [isinstance(elem, pd.DataFrame) for elem in mark_events]
            ):
                assert all(
                    [
                        all([col in elem.columns for col in ["station", "time"]])
                        for elem in mark_events
                    ]
                ), (
                    "All DataFrames in "
        ...
        def update_timeseries(event, select_station=None):
            ...
            if select_station is None:
                if event is not None:
                    if (
                        ...
                        return
                    click_lon, click_lat = proj_lla.transform_point(
                        event.xdata, event.ydata, src_crs=proj_gui
                    )
                    distances = geoid.inverse(
                        np.array([[click_lon, click_lat]]), stat_lonlats
                    )
            ...
            ts_to_plot = {
                ts_description: ts
                for ts_description, ts in self[station_name].timeseries.items()
            ...
            for ts_description, ts in ts_to_plot.items():
                ...
                if analyze_kw_args:
                    self[station_name].analyze_residuals(
                        ts_description, **analyze_kw_args
                    )
            ...
            for ts_description, ts in ts_to_plot.items():
                for icol, (data_col, var_col) in enumerate(
                    zip(
                        ts.data_cols,
                        (
                    ...
                    if (var_col is not None) and (gui_settings["plot_sigmas"] > 0):
                        fill_upper = ts.df[data_col] + gui_settings["plot_sigmas"] * (
                            ts.df[var_col] ** 0.5
                        )
                        fill_lower = ts.df[data_col] - gui_settings["plot_sigmas"] * (
                            ts.df[var_col] ** 0.5
                        )
                        ax.fill_between(
                            ts.time,
                    ...
                    if (
                        sum_models
                        and (fit_list is None)
                        and self[station_name].fits[ts_description].allfits
                    ):
                        # get joint model
                        fit = self[station_name].fits[ts_description].allfits
                        # plot all at once
                        if (fit.var_cols is not None) and (
                            gui_settings["plot_sigmas"] > 0
                        ):
                            fill_upper = fit.df[fit.data_cols[icol]] + gui_settings[
                                "plot_sigmas"
                            ] * (fit.df[fit.var_cols[icol]] ** 0.5)
                            fill_lower = fit.df[fit.data_cols[icol]] - gui_settings[
                                "plot_sigmas"
                            ] * (fit.df[fit.var_cols[icol]] ** 0.5)
                            ax.fill_between(
                                fit.time,
                        ...
                        fits_to_plot = {
                            ...
                            for model_description, fit in self[station_name]
                            .fits[ts_description]
                            .items()
                        ...
                        if sum_var is not None:
                            fill_upper = (
                                sum_fit + (sum_var**0.5) * gui_settings["plot_sigmas"]
                            )
                            fill_lower = (
                                sum_fit - (sum_var**0.5) * gui_settings["plot_sigmas"]
                            )
                    ...
                    if scalogram_kw_args is not None:
                        t_left = (
                            ts.time[0] if t_left is None else min(ts.time[0], t_left)
                        )
                        t_right = (
                            ts.time[-1]
                            if t_right is None
                            else max(ts.time[-1], t_right)
                        )
            ...
            if mark_events is not None:
                ...
                # loop over axes
                ax_t_min, ax_t_max = (
                    mpl.dates.num2date(ax_time).replace(tzinfo=None)
                    for ax_time in ax_ts[0].get_xlim()
                )
                mark_subset = mark_subset[
                    (mark_subset["time"] >= ax_t_min)
            ...
            if stepdetector:
                ...
                station_steps = step_table[
                    (step_table["time"] >= sub_tmin) & (step_table["time"] <= sub_tmax)
                ]["station"].tolist()
                print(
                ...
                sub_step = step_table[
                    (step_table["station"] == station_name)
                ...
                if maint_table is None:
                    ...
                    sub_maint = maint_table[
                        (maint_table["station"] == station_name)
                ...
                if catalog is None:
                    ...
                    sub_cat = catalog[
                        (catalog["Date_Origin_Time(JST)"] >= sub_tmin)
                        & (catalog["Date_Origin_Time(JST)"] <= sub_tmax)
                    ]
                    if not sub_cat.empty:
                        sub_lonlats = np.hstack(
                            (
                                sub_cat["Longitude(°)"].values.reshape(-1, 1),
                        ...
                        distances = geoid.inverse(
                            station_lonlat.reshape(1, 2), sub_lonlats
                        )
                        ...
                        sub_cat = sub_cat.iloc[
                            np.any(
                                np.stack([distances <= eqcircle, large_ones]), axis=0
                            ),
                            :,
                        ].copy()
                        # print
                        if OKADA_LOADED and (not sub_cat.empty):
                            ...
                            for i in range(sub_cat.shape[0]):
                                station_disp[i, :] = okada_displacement(
                                    self[station_name].location, sub_cat.iloc[i, :]
                                )
                            sub_cat["Okada_Estim_Mag(mm)"] = np.sqrt(
                                np.sum(station_disp**2, axis=1)
                            )
                            print("Related earthquakes:")
                            print(
                                sub_cat[
                                    [
                                        "Date_Origin_Time(JST)",
                                        "Latitude(°)",
                                        "Longitude(°)",
                                        "MT_Depth(km)",
                                        "MT_Magnitude(Mw)",
                ...
                for ax in ax_ts:
                    ...
                    for line in ax.get_lines():
                        ...
                        tsubset = (pd.Series(xdata) >= xlow) & (
                            pd.Series(xdata) <= xhigh
                        )
                    ...
                    ax.axvspan(
                        sub_tmin, sub_tmax, ec=None, fc="C6", alpha=0.1, zorder=-100
                    )
            ...
            if save and not stepdetector:
                ...
                plotfname = (
                    f"{station_name}{fname_add}_{nowtime}.{save_kw_args['format']}"
                )
            ...
            if scalogram_kw_args is not None:
                try:
                    ...
                    if isinstance(fig_scalo, plt.Figure):
                        plt.close(fig_scalo)
                    fig_scalo, ax_scalo = splset.make_scalogram(
                        t_left, t_right, **scalogram_kw_args  # noqa: F841
                    )
                    # save figure or show
                    if save and not stepdetector:
                    ...
                    warn(
                        f"Could not find scalogram model {scalo_model} "
    ...
    def wormplot(
        ...
        fname
            If set, save the map to this filename, if not (default), show the map
            interactively (unless ``return_figure=True``).
        fname_animation
        ...
        lat_max
            Specify the map's maximum latitude (in degrees).
        en_col_names
            By default, the first two components of the timeseries will be assumed to be
            the East and North components, respectively, by having the default value
            ``(0, 1)`` indicating the desired components as integer indices.
            Alternatively, this can be a tuple of strings with the two component's
            column names.
        scale
            ...
            in millimeters and a scale of ``1e2``, one millimeter displacement will
            result in a mapped displacement of 100 meters.
        interval
        ...
        save_kw_args
            Additional keyword arguments passed to
            :meth:`~matplotlib.figure.Figure.savefig`, used when ``fname`` is specified.
        colorbar_kw_args
            If ``None``, no colorbar is added to the plot. If a dictionary is passed,
            a colorbar is added, with the dictionary containing additional keyword
            arguments to the :meth:`~matplotlib.figure.Figure.colorbar` method.
        legend_ref_dict
            If ``legend_ref_dict`` is provided and contains all of the following
            information, a reference line will be plotted at the specified location to
            act as a legend. Necessary keys: ``location`` (a tuple containing the
            longitude and latitude of the reference line), ``length`` (the length of the
            line in meters) and ``label`` (a label placed below the line).
            The dictionary can optionally include ``rect_args`` and ``rect_kw_args``
            entries for arguments and parameters that will be passed onto the creation
            of the background :class:`~matplotlib.patches.Rectangle`.
        gui_kw_args
            Override default GUI settings of :attr:`~disstans.config.defaults`.
        ...
        if (
            isinstance(en_col_names, tuple)
            and (len(en_col_names) == 2)
            and all([isinstance(comp, int) for comp in en_col_names])
        ):
            col_east, col_north = np.array(list(network_df.keys()))[
                list(en_col_names)
            ].tolist()
        elif (
            isinstance(en_col_names, tuple)
            and (len(en_col_names) == 2)
            and all([isinstance(comp, str) for comp in en_col_names])
        ):
            col_east, col_north = en_col_names
        else:
            raise ValueError(
                "'en_col_names' needs to be a two-element tuple of integers or "
                f"strings, got {en_col_names}."
            )

        # for each station, get displacement timeseries for the east and north
        # components; these two dataframes will have a column for each station
        ...
        geoid = cgeod.Geodesic()
        stat_lonlats = {
            name: np.array(self[name].location)[[1, 0]].reshape(1, 2)
            for name in rel_disp_x.keys()
        }
        azi = {
            name: 90 - np.arctan2(rel_disp_y[name], rel_disp_x[name]) * 180 / np.pi
            for name in rel_disp_x.keys()
        }
        dist = {
            name: np.sqrt(rel_disp_x[name] ** 2 + rel_disp_y[name] ** 2)
            for name in rel_disp_x.keys()
        }
        disp_latlon = {
            name: np.array(
                geoid.direct(stat_lonlats[name], azi[name], dist[name] * scale)[:, :2]
            )
            for name in rel_disp_x.keys()
        }
        lines = {
        ...
        if gui_settings["wmts_show"]:
            try:
                src = WMTSRasterSource(
                    wmts=gui_settings["wmts_server"],
                    layer_name=gui_settings["wmts_layer"],
                )
                bbox = ax_map.get_window_extent().transformed(
                    fig_map.dpi_scale_trans.inverted()
                )
                width_height_px = round(bbox.width * fig_map.dpi), round(
                    bbox.height * fig_map.dpi
                )
        ...
        if isinstance(colorbar_kw_args, dict):
            ...
            cbar = fig_map.colorbar(
                ScalarMappable(norm=Normalize(vmin=0, vmax=1), cmap=scm.batlow),
                ax=ax_map,
                **colorbar_kw_args,
            )
            cbar.ax.set_xticklabels(
                [str(disp_x.time[0].date()), str(disp_x.time[-1].date())]
            )
        ...
        if isinstance(legend_ref_dict, dict):
            if not all([k in legend_ref_dict for k in ["location", "length", "label"]]):
                print(
                    "'legend_ref_dict' does not contain all "
                    "valid keys to create a legend."
                )
            else:
                ref_disp = np.linspace(0, legend_ref_dict["length"], num=num_timesteps)
                ref_lonlat = np.array(legend_ref_dict["location"]).reshape(1, 2)
                ref_loc = np.array(
                    geoid.direct(ref_lonlat, 90, ref_disp * scale)[:, :2]
                )
                ax_map.scatter(
                    ref_loc[:, 0],
                    ref_loc[:, 1],
                    facecolor=relcolors[:, :3],
                    edgecolor="none",
                    zorder=10,
                    transform=proj_lla,
                    rasterized=True,
                ...
                ax_map.annotate(
                    legend_ref_dict["label"],
                    ref_loc[num_timesteps // 2, :],
                    xytext=(0, -6),
                ...
                if ("rect_args" in legend_ref_dict) and (
                    "rect_kw_args" in legend_ref_dict
                ):
                    ax_map.add_patch(
                        Rectangle(
                            *legend_ref_dict["rect_args"],
    ...
    def ampphaseplot(
        ...
        month_range
            ...
            Pass a different range (in inclusive months, e.g., ``(5, 10)`` for all of
            May through all of October) to subset the range.
        ...
        colorbar_kw_args
            ...
            If ``None`` (default), no colorbar is added to the plot. If a dictionary is
            passed, a colorbar is added, with the dictionary containing additional
            keyword arguments to the :meth:`~matplotlib.figure.Figure.colorbar` method.
        gui_kw_args
            Override default GUI settings of :attr:`~disstans.config.defaults`.
        ...
        for stat_name in subset_stations:
            ...
            try:
                ...
                if not isinstance(mdl, disstans_models.Sinusoid):
                    raise RuntimeError(
                        "'mdl_description' refers to a model object that "
        ...
        if month_range is None:
            ...
            assert (
                isinstance(month_range, tuple)
                and len(month_range) == 2
                and all([isinstance(m, int) and (1 <= m <= 12) for m in month_range])
            ), f"'month_range' must be a tuple of 2 integer months, got {month_range}."
            cmap = scm.batlow
            vmin = month_range[0]
            vmax = month_range[1] + 1 - 1e-3
            phase_cols = (
                (np.array(phases) % (2 * np.pi)) / (2 * np.pi) * 12 - month_range[0]
        ...
        if isinstance(legend_refs, list):
            ...
            ref_handles = ref_scatter.legend_elements(
                ...
                markeredgecolor="k",
                visible=True,
                func=lambda s: np.sqrt(s / (20 * scale)),
            )[0]
    ...
    def plot_availability(
        ...
        if sort_stations is None:
            sort_stations = list(
                reversed(sorted([s.lower() for s in self.station_names]))
            )
