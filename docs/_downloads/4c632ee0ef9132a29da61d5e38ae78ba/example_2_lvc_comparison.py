...
if __name__ == "__main__":
    ...
    all_poly_df = pd.read_csv(
        f"{out_dir_ex1}/example_1_secular_velocities.csv", index_col=0
    )
    poly_stat_names = all_poly_df.index.tolist()
    print("Done")

    # get predicted velocities from uniform motion & deformation
    v_pred_hom, v_O, epsilon, omega = net.hom_velocity_field(
        ts_description="final", mdl_description="Linear"
    )
    ...
    v_pred_euler, rotation_vector, rotation_covariance = net.euler_rot_field(
        ts_description="final", mdl_description="Linear"
    )

    # get Euler pole notation
    euler_pole, euler_pole_covariance = rotvec2eulerpole(
    ...
    if not fname_midas.is_file():
        url_midas = "https://geodesy.unr.edu/velocities/midas.IGS14.txt"
    ...
    v_mdl["DISSTANS"] = (
        all_poly_df.loc[
            common_stations, ["vel_e", "vel_n", "sig_vel_e", "sig_vel_n"]
        ].copy()
        / 1000
    )
    v_mdl["DISSTANS"]["corr_vel_en"] = all_poly_df.loc[
        common_stations, ["corr_vel_en"]
    ].copy()

    # also convert the MIDAS and GAGE velocity dataframes into the same format
    v_mdl["MIDAS"] = pd.DataFrame(
        {
            "vel_e": v_mdl_midas.loc[common_stations, "ve50"],
    ...
    v_mdl["GAGE"] = pd.DataFrame(
        {
            "vel_e": v_mdl_gage.loc[common_stations, "dE/dt"],
    ...
    for station in tqdm(
        common_stations, desc="Building networks", unit="station", ascii=True
    ):
        # create stations
        ...
        stat_vel_midas = (
            v_mdl_midas.loc[station, ["ve50", "vn50", "vu50"]]
            .to_numpy(dtype=float)
            .reshape(1, -1)
        )
        stat_var_midas = (
            v_mdl_midas.loc[station, ["sve", "svn", "svu"]]
            .to_numpy(dtype=float)
            .reshape(1, -1)
            ** 2
        )
        ...
        stat_vel_gage = (
            v_mdl_gage.loc[station, ["dE/dt", "dN/dt", "dU/dt"]]
            .to_numpy(dtype=float)
            .reshape(1, -1)
        )
        stat_sd_gage = (
            v_mdl_gage.loc[station, ["SEd", "SNd", "SUd"]].to_numpy(dtype=float).ravel()
        )
        stat_corr_gage = (
            v_mdl_gage.loc[station, ["Rne", "Reu", "Rnu"]].to_numpy(dtype=float).ravel()
        )
    ...
    print(
        "Estimating homogenous velocity strain & rotation field... ", end="", flush=True
    )
    v_pred, v_pred_os = {}, {}
    for case in ["DISSTANS", "MIDAS", "GAGE"]:
        v_pred[case] = net_comp.euler_rot_field(
            ts_description="final",
            mdl_description=case,
            subset_stations=common_stations,
            extrapolate=common_stations,
        )[0]
        v_pred_os[case] = net_comp.euler_rot_field(
            ts_description="final",
            mdl_description=case,
    ...
    v_mdl_na = {
        case: (-v_NA_enu.iloc[:, :2]) + v_mdl[case][["vel_e", "vel_n"]].values
    ...
    v_mdl_covs = {
        case: np.stack(
            [
                v_mdl[case]["sig_vel_e"].values ** 2,
                v_mdl[case]["sig_vel_n"].values ** 2,
                np.prod(
                    v_mdl[case][["sig_vel_e", "sig_vel_n", "corr_vel_en"]].values,
                    axis=1,
                ),
            ],
            axis=1,
    ...
    for case in ["DISSTANS", "MIDAS", "GAGE"]:
        ...
        rotation_vector = estimate_euler_pole(
            station_lolaal[:, :2], v_mdl_na[case].values, v_mdl_covs[case]
        )[0]
    ...
    for case in ["DISSTANS", "MIDAS", "GAGE"]:
        rotation_vector = estimate_euler_pole(
            station_lolaal[outer_shell, :2],
            v_mdl_na[case].values[outer_shell, :],
            v_mdl_covs[case][outer_shell, :],
        )[0]
        v_temp = np.cross(rotation_vector, stat_xyz)
    ...
    rmses_predmdls = pd.DataFrame(
        {
            "D2M": [
                (
                    np.linalg.norm(vp["DISSTANS"].values - vp["MIDAS"].values, axis=1)
                    ** 2
                ).mean()
                ** 0.5
                for vp in [v_pred, v_pred_os]
            ],
            "D2G": [
                (
                    np.linalg.norm(vp["DISSTANS"].values - vp["GAGE"].values, axis=1)
                    ** 2
                ).mean()
                ** 0.5
                for vp in [v_pred, v_pred_os]
            ...
            "M2G": [
                (
                    np.linalg.norm(vp["MIDAS"].values - vp["GAGE"].values, axis=1) ** 2
                ).mean()
                ** 0.5
                for vp in [v_pred, v_pred_os]
            ],
    ...
    rmses_res = pd.DataFrame(
        np.zeros((3, 3)), columns=list(v_res.keys()), index=["all", "outer", "inner"]
    )
    rmses_res_os, rmses_resdiff = rmses_res.copy(), rmses_res.copy()
    for sub_name, sub_ix in zip(
        ["all", "outer", "inner"],
        ...
        rmses_res_os.loc[sub_name, :] = [
            (np.linalg.norm(v_res_os[case].values[sub_ix, :], axis=1) ** 2).mean()
            ** 0.5
        ...
        rmses_resdiff.loc[sub_name, :] = [
            (
                np.linalg.norm(
                    v_res[case].values[sub_ix, :] - v_res_os[case].values[sub_ix, :],
                    axis=1,
                )
                ** 2
            ).mean(axis=0)
